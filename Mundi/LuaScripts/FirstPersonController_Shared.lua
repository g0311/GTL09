-- ==================== First Person Controller (Shared Variables) ====================
-- WASD 이동 + 마우스 카메라 회전을 통합한 1인칭 컨트롤러
-- 다른 스크립트에서 플레이어 속도를 읽을 수 있도록 전역 테이블에 저장
--
-- 사용법:
...
-- ==================== 전역 공유 데이터 ====================
-- _G는 Lua의 전역 테이블입니다
-- 주의: 각 ScriptComponent가 독립적인 Lua state를 가지므로
-- 이 방법은 같은 state 내에서만 작동합니다.
-- 대신 Actor를 통해 접근하는 방법을 사용해야 합니다.
...
---
--- 다른 스크립트에서 플레이어 속도를 가져올 수 있는 함수
--- 외부에서 호출: playerActor:GetComponent("ScriptComponent"):CallLuaFunction("GetMoveSpeed")
---
